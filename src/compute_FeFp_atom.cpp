...
//  double **neighlist0 = fix2->array_atom;
  ...
  for(i=0;i<nlocal;i++){
     ...
     for(j=0;j<nneigh;j++){
        ...
        if(n_id<0) printf("wrong id for atom %d neigh %d, id_global %d, id_local %d check neighlist build\n",i,j,n_id_global,n_id);
        xn[0] = xa[n_id][0] ;
        xn[1] = xa[n_id][1] ;
        xn[2] = xa[n_id][2] ;
	...
	dist0[2] = neighdist[i][j*4+3];
        ...
        // for some atoms ( neighbor of which has crossed boundary), detected by some thing)

//        distT[0] = xn[0] - xc[0]-dxprd;
//        distT[1] = xn[1] - xc[1]-dyprd;
//        distT[2] = xn[2] - xc[2]-dzprd;
...
//           else{printf("plastic bound neigh %d, cross %d, distT %f, dist0 %f, xn %f, xc %f\n",j,cross,distT[1],dist0[1],xn[1],xc[1]);}

...
int FixAveAtom::pack_comm(int n, int *list,double *buf,int pbc_flag, int *pbc)
{
    int i,j,m;
    int k;
    m = 0;
    for(i=0;i<n;i++){
       j = list[i];
       for(k=0;k<nvalues;k++){
          buf[m++] = vector_atom[j*size+k];
       }
    }
    return nvalues;   
}

void FixAveAtom::unpack_comm(int n, int first, double *buf)
{
    int i,m,last;
    int k,size;
    m = 0;

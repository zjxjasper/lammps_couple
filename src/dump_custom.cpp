...
enum{ID,MOL,TYPE,ELEMENT,MASS,
     ...
     IX,IY,IZ,
     VX,VY,VZ,FX,FY,FZ,FEXTX,FEXTY,FEXTZ,IVCID,
  ...
  Dump(lmp, narg, arg)
{
  ...
  if (ioptional < narg && 
     ...
     printf("check 2 in dump custom %d %d\n",ioptional,narg);
  ...
  if (nthresh) {
    ...
    for (int ithresh = 0; ithresh < nthresh; ithresh++) {
      ...
      if (thresh_array[ithresh] == ID) {
        ...
        ptr = &atom->f[0][2];
        nstride = 3;
      } else if (thresh_array[ithresh] == FEXTX) {
        ptr = &atom->fext[0][0];
        nstride = 3;
      } else if (thresh_array[ithresh] == FEXTY) {
        ptr = &atom->fext[0][1];
        nstride = 3;
      } else if (thresh_array[ithresh] == FEXTZ) {
        ptr = &atom->fext[0][2];
        nstride = 3;
      /******added for IVC_ID******/
      } else if (thresh_array[ithresh] == IVCID) {
        int *IVC_id = atom->IVC_id;
        for (i = 0; i < nlocal; i++) dchoose[i] = IVC_id[i];
        ptr = dchoose;
        nstride = 1;
      /****** IVC_ID end******/
...
int DumpCustom::parse_fields(int narg, char **arg)
{
  ...
  for (int iarg = 5; iarg < narg; iarg++) {
    ...
    if (strcmp(arg[iarg],"id") == 0) {
      ...
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"fextx") == 0) {
      pack_choice[i] = &DumpCustom::pack_fextx;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"fexty") == 0) {
      pack_choice[i] = &DumpCustom::pack_fexty;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"fextz") == 0) {
      pack_choice[i] = &DumpCustom::pack_fextz;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"ivc") == 0) {
      pack_choice[i] = &DumpCustom::pack_ivc;
      vtype[i] = INT;
...
int DumpCustom::modify_param(int narg, char **arg)
{
  ...
  if (strcmp(arg[0],"thresh") == 0) {
    ...
    else if (strcmp(arg[1],"fextx") == 0) thresh_array[nthresh] = FEXTX;
    else if (strcmp(arg[1],"fexty") == 0) thresh_array[nthresh] = FEXTY;
    else if (strcmp(arg[1],"fextz") == 0) thresh_array[nthresh] = FEXTZ;
    else if (strcmp(arg[1],"ivc") == 0)   thresh_array[nthresh] = IVCID;
...
/* ---------------------------------------------------------------------- */
void DumpCustom::pack_fextx(int n)
{
  double **fext = atom->fext;
  
  for (int i = 0; i < nchoose; i++) {
    buf[n] = fext[clist[i]][0];
//   if(isnan(buf[n])) printf("in dump detect nan, i %d n %d\n",i,n);
    n += size_one;
  }
}

/* ---------------------------------------------------------------------- */
void DumpCustom::pack_fexty(int n)
{
  double **fext = atom->fext;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = fext[clist[i]][1];
    n += size_one;
  }
}

/* ---------------------------------------------------------------------- */
void DumpCustom::pack_fextz(int n)
{
  double **fext = atom->fext;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = fext[clist[i]][2];
    n += size_one;
  }
}

/* ---------------------------------------------------------------------- */
void DumpCustom::pack_ivc(int n)
{
  int *IVC_id = atom->IVC_id;

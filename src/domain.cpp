...
Domain::~Domain()
{
  ...
  if(flag_natoms_IVC == 1){
     memory->destroy(natoms_IVC);
     memory->destroy(natoms_SVC);
  }
}
...
void Domain::create_natoms_IVC(int size)
{
    if(flag_natoms_IVC == 1) return;
    else{
      memory->create(natoms_IVC,size,"domain::natoms_IVC");
      memory->create(natoms_SVC,size,"domain::natoms_SVC");
...
void *Domain::extract(char *name)
{
  if (strcmp(name,"natoms_IVC") == 0) return (void *) natoms_IVC;
  if (strcmp(name,"natoms_SVC") == 0) return (void *) natoms_SVC;
  return NULL;
}
